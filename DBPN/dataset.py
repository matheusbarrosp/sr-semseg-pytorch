...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ix = -1
        iy = -1
        if input.size[0] == self.patch_size and input.size[1] == self.patch_size:
            ix = 0
            iy = 0

        input, target, bicubic, _ = get_patch(input,target,bicubic,self.patch_size, self.upscale_factor, ix, iy)
